...
# option num_threads=* -pe pe_mpi $0
...
for (my $x = 1; $x <= 2; $x++) { # This for-loop is to
  ...
  while (@ARGV >= 2 && $ARGV[0] =~ m:^-:) {
    ...
    if ($switch eq "-V") {
      ...
      } elsif ($switch eq "-pe") { # e.g. -pe pe_mpi 5
...
while(<CONFIG>) {
  ...
  if ($_ =~ /^command (.+)/) {
    ...
    # e.g.: option mem=* -l mem_free=$0
    my $option = $1;     # mem
    my $arg= $2;         # -l mem_free=$0
    if ($arg !~ m:\$0:) {
    ...
    if (exists $cli_options{$option}) {
      ...
      # e.g. "-l mem_free=$0" -> "-l mem_free=2G"
